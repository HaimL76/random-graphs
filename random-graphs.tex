...
Following the construction by Tutte, $\forall{v}\in{V}$, $f(v)=2$, and we construct $St(v)=X(v)\cup{Y}(v)$, where $X(v)$ has $d(v)$ distinct vertices and $Y(v)$ has $s(v)=d(v)-f(v)=d(v)-2$ distinct vertices, and we construct a new graph $G'=(V',E')$, s.t. \[V'=\bigcup_{v\in{V}}(X(v)\cup{Y}(v),\] 
...
We find a perfect matching $M$ on $G'$, then $M$ is a spanning subgraph of $G'$ (because every vertex $v\in{V}$ must be an endpoint of only one edge from $G'$, hence $M$ spans all $G'$ vertices, and the number of edges in $M$ is less or equal to that of $G'$).

 Let $u\in{V}$ be another vertex, s.t. $v$ and $u$ are adjacent, then we match one of the unmatched vertices in $X(v)$ with one of the unmatched vertices in $X(u)$.
